...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        if(channel){
       channel.send(`╔════════════「 **Gezend | Davet Sistemi** 」════════════╗ \n║\n║<a:sunucuyakatld:774199578508918806>  ・  ${member} **__Adlı Kişi Sunucuya Katıldı__** \n║\n║<a:yumuakok:770591161672531978>  ・  **__Davet Eden Şahıs:__** ${invite.inviter.tag} \n║\n║<a:mormaden:770588946132238336>  ・  **__Toplam__** **${total + bonus}** **__Daveti Oldu__** \n║\n╚════════════「 **Gezend | Davet Sistemi** 」════════════╝`)
        }
    }).catch();
});
    ...
    db.add(`invites.${data.inviter}.leave`, 1);
     if(channel){
        let user = client.users.cache.get(data.inviter)
     	channel.send(`╔════════════「 **Gezend | Davet Sistemi** 」════════════╗ \n║\n║<a:sunucudanayrld:774199640933269514>  ・  ${member.user.tag} **__Adlı Kişi Sunucudan Ayrıldı__** \n║\n║<a:yumuakok:770591161672531978>  ・  **__Davet Eden Şahıs:__** ${user.tag} \n║\n║<a:mormaden:770588946132238336>  ・  **__Toplam__** **${Number(total) + Number(bonus)}** **__Daveti Kaldı__** \n║\n╚════════════「 **Gezend | Davet Sistemi** 」════════════╝`)
     }
});
